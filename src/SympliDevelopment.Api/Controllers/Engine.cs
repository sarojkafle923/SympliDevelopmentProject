    ...
    public class Engine
    {
        ...
        public Result[] Search(string query, int start = 0)
        {
            ...
            try
            {
                ...
                {
                    if (node.GetClasses().Contains("yuRUbf")) //Check If The Node Has The Class "ZINbbc"
                        if (node.FirstChild.OuterHtml.StartsWith("<a href=\"https://www.sympli.com.au/\"")) //Check If The Link(Inside The Node)'s Outer HTML Starts With "<a href="/url?q="
                        {
